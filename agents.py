...
def nuclear_cost(nuclear_plantlife,nuclear_efficiency,nuclear_capitalcost,nuclear_operationscost,nuclear_varible_cost,nuclear_fuelcost,nuclear_heatrate):
...
nuclear_result = nuclear_cost(nuclear_plantlife, nuclear_efficiency, nuclear_capitalcost, nuclear_operationscost, nuclear_variablecost, nuclear_fuelcost, nuclear_heatrate)
...
def coal_cost(coal_plantlife, coal_efficiency, coal_capitalcost, coal_operationscost, coal_variablecost, coal_fuelcost, coal_heatrate):
...
coal_result = coal_cost(coal_plantlife, coal_efficiency, coal_capitalcost, coal_operationscost, coal_variablecost, coal_fuelcost, coal_heatrate)
...
def gas_cost(gas_plantlife, gas_efficiency, gas_capitalcost, gas_operationscost, gas_variablecost, gas_fuelcost, gas_heatrate):
...
gas_result = gas_cost(gas_plantlife, gas_efficiency, gas_capitalcost, gas_operationscost, gas_variablecost, gas_fuelcost, gas_heatrate)
...
coal_capacities = [(random.normalvariate(397,7)),(random.normalvariate(397,7)),(random.normalvariate(397,7)),(random.normalvariate(397,7)),]
coal_total_capacity = sum(coal_capacities)
coal_monthsrunning = [(random.normalvariate(480,60)),(random.normalvariate(480,60)),(random.normalvariate(480,60)),(random.normalvariate(480,60))]
...
gas_capacities = [(random.normalvariate(578,426)),(random.normalvariate(578,426)),(random.normalvariate(578,426)),(random.normalvariate(578,426)),(random.normalvariate(578,426)),(random.normalvariate(578,426)),(random.normalvariate(578,426)),(random.normalvariate(578,426)),(random.normalvariate(578,426)),(random.normalvariate(578,426)),(random.normalvariate(578,426)),(random.normalvariate(578,426)),(random.normalvariate(1397,168.5)),(random.normalvariate(1397,168.5)),(random.normalvariate(1397,168.5)),(random.normalvariate(1397,168.5))]
gas_total_capacity = sum(nuclear_capacities)
gas_monthsrunning = [(random.normalvariate(186,54)),(random.normalvariate(186,54)),(random.normalvariate(186,54)),(random.normalvariate(186,54)),(random.normalvariate(186,54)),(random.normalvariate(186,54)),(random.normalvariate(186,54)),(random.normalvariate(186,54)),(random.normalvariate(186,54)),(random.normalvariate(186,54)),(random.normalvariate(186,54)),(random.normalvariate(186,54)),(random.normalvariate(186,54)),(random.normalvariate(186,54)),(random.normalvariate(186,54)),(random.normalvariate(186,54))]

def gas_generation(gas_total_capacity, gas_efficiency):
    return float((gas_total_capacity * 24 * 30.416 * gas_efficiency*.8)+(gas_total_capacity*24*30.416*.12*.12))
...
total_clean_gen = ((wind_generation(wind_total_capacity, wind_efficiency))+ (solar_generation(solar_total_capacity, solar_efficiency))+(hydrocreate_generation(hydrocreate_total_capacity,hydrocreate_efficiency)))
...
#To kWh Costs
residential_kWh_cost = 1.8*((nuclear_total_capacity*nuclear_cost)+(coal_total_capacity*coal_result)+(gas_total_capacity*gas_result)+(wind_total_capacity*wind_cost)+(solar_total_capacity*wind_cost))
commercial_kWh_cost = 1.5*((nuclear_total_capacity*nuclear_cost)+(coal_total_capacity*coal_result)+(gas_total_capacity*gas_result)+(wind_total_capacity*wind_cost)+(solar_total_capacity*wind_cost))
...
class Residential(Agent):
    money=np.random.lognormal(75000,25000)
    wtp = random.normalvariate(16,16)
    energy_usage = random.normalvariate(696,100)
    savings = np.random.lognormal(70000,70000)
    costs = energy_usage * residential_kWh_cost
    #whats up with solar and clean.


    def __init(self,unique_id, pos, model, money =np.random.lognormal(75000,25000),wtp = random.normalvariate(16,16),energy_usage = random.normalvariate(696,100), savings = np.random.lognormal(70000,70000),costs = (energy_usage * residential_kWh_cost)):
...
class Commercial(Agent):
    money = np.random.lognormal(300000,150000)
    wtp = random.normalvariate(200, 200)
    energy_usage = random.normalvariate(6300, 1000)
    savings = np.random.lognormal(3000000, 1000000)
    costs = energy_usage * commercial_kWh_cost

    # whats up with solar and clean.

    def __init(self, unique_id, pos, model, money=np.random.lognormal(300000,150000),wtp=random.normalvariate(200, 200), energy_usage=random.normalvariate(6300, 1000),savings=np.random.lognormal(3000000, 1000000), costs=(energy_usage * commercial_kWh_cost)):
        super().__init__(unique_id, pos, model)
        self.money = money
        self.wtp = wtp
        self.energy_usage = energy_usage
...
#     """
# #put all initial variables differnt for each agent that change each step)
#     energy = None
#
...
#
#         A model step.what happens to the residnetial and commercial agents each step...what changes. This is what we need to define here
#         """
#         #they can move but should not affect anything might not even need move but rather death/population growth
#         self.random_move()
#         living = True
#
#         if self.model.grass:
